...
if __name__=='__main__':
    ...
    # Internal L (sets computation accuracy, LMAX = 8 is optimal, but LMAX ~ 4 may be okay)
    LMAX = int(sys.argv[1])
    nu = float(sys.argv[2])
    cs = float(sys.argv[3])
    ...
    h = 0.6777
    ns = 1
    DeltaSq_zeta = 4.1e-8 # primordial amplitude (from Planck)
    ...
    Pzeta = lambda k: DeltaSq_zeta/k**3
    ...
    from scipy.special import hyp2f1, gamma, legendre
    ...
    Pzeta_arr = Pzeta(k_arr)
    kM_arr = ksq_arr*Mk_arr
    ...
    tA_integ = (Pzeta(k1_arr)*k1_arr)*(Pzeta(k2_arr)*k2_arr**2)*(k1_arr-k2_arr)*(k12_arr*J3_arr+cs*k1_arr*k2_arr*J4_arr)
    tB_integ = (Pzeta(k1_arr)*k1_arr**2)*(Pzeta(k2_arr)*k2_arr**2)*(k1_arr-k2_arr)*(k12_arr*J4_arr+cs*k1_arr*k2_arr*J5_arr)
