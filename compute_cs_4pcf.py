### Compute the Odd-Parity 4PCF arising from Chern-Simons coupling as in Philcox 2021
# The output contains the 4PCF multiplets, but without a prefactor of -1*A_{CS}
...
def kernel1(L1,L2,L3,L4,L5,L5p):
    """Load part of coupling matrix which is ell-independent."""
    ...
    lam2s = [1,2,1,2]
    lam3s = [1,1,2,2]
    lamWs = [1.,1./np.sqrt(5.),1./np.sqrt(5.),-1./np.sqrt(5.)]
    ...
    s_weight = np.bincount(inv_idx, weights=s_weight)
...
outfile = '/home/ophilcox/Parity-Odd-4PCF/cs_4pcf_L%d.txt'%LMAX
np.savetxt(outfile,sum_output)
